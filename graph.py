...
class NeighborFinder:
    def __init__(self, adj_list, walk_type, bias=0, ts_precision=PRECISION, sample_method='multinomial'):
    ...
    def compute_binary_prob(self, ts_l):
        ...
        exp_input = np.clip(self.bias * ts_l, -700, None)
        exp_ts_l = np.exp(exp_input)

        cumsum = np.cumsum(exp_ts_l)       
        cumsum_safe = np.where(cumsum == 0, 1e-10, cumsum)
        exp_ts_l /= cumsum_safe #np.cumsum(exp_ts_l) 
        return exp_ts_l
    
    def compute_binary_prob_original(self, ts_l):
        if len(ts_l) == 0:
            return np.array([])
        ts_l = ts_l - np.max(ts_l) 
        exp_ts_l = np.exp(self.bias*ts_l)  
        cumsum = np.cumsum(exp_ts_l)       
        exp_ts_l /= cumsum #np.cumsum(exp_ts_l) 
        return exp_ts_l
    ...
    def find_before(self, src_idx, cut_time, e_idx=None, return_binary_prob=False):
        ...
        if not return_binary_prob: # multinomial
    ...
    def get_temporal_neighbor(self, src_idx_l, cut_time_l, num_neighbor, e_idx_l=None):        
        out_ngh_node_batch = np.full((len(src_idx_l), num_neighbor), PADDING_NODE_ID).astype(np.int32)
        out_ngh_t_batch = np.full((len(src_idx_l), num_neighbor), PADDING_TIMESTAMP).astype(np.float32)
        out_ngh_eidx_batch = np.full((len(src_idx_l), num_neighbor), PADDING_EDGE_ID).astype(np.int32)
        ...
        for i, (src_idx, cut_time) in enumerate(zip(src_idx_l, cut_time_l)):
            ...
            if ngh_binomial_prob is None: 
                if math.isclose(self.bias, 0):
                    ...
                    time_delta = cut_time.item() - ngh_ts
    ...
    def find_k_hop(self, k, src_idx_l, cut_time_l, num_neighbors):
        ...
        for layer_i in range(1, k):
            ...
            if self.walk_type == 'before':
                out_ngh_node_batch, out_ngh_eidx_batch, out_ngh_t_batch = self.get_temporal_neighbor(ngh_node_est, ngh_t_est, 1)
    ...
    def get_random_walk(self, src_idx, cut_time, seed=0, len_walk=5, e_idx=None, packed=False, recent_bias=1.0):
        ...
        if packed:
            ...
            for hop in range(len_walk):
                ...
                cur_n_idx, cur_time, cur_e_idx = n_idx_l[idx_picked], ts_l[idx_picked], e_idx_l[idx_picked] 
